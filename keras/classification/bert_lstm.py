...
def make_model(config):
    ...
    output = Bidirectional(LSTM(128, dropout=0.5, recurrent_dropout=0.2))(output)
    output = Dropout(rate=0.1)(output)
    output = Dense(
        units=config['class_num'], activation=config['activation'], kernel_initializer=bert.initializer)(output)
...
if __name__ == "__main__":
    parser = create_parser(description='基于Bert+LSTM的文本分类')
    ...
    trainer = Trainer(model_name='classification_bert_lstm', dataset=dataset, tokenizer=tokenizer, model=model)
